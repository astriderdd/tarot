...
export function getCard(cardIndex: number): Card {
  // STEP 1: Try to get the card from our cards array using the index
  // Array access: cards[cardIndex] gets the card at that position
  // - If cardIndex is 0, we get cards[0] (The Fool)
  // - If cardIndex is 42, we get cards[42] (some other card)
  // - If cardIndex is 999, we get undefined (card doesn't exist)
  const card = cards[cardIndex]

  // STEP 2: Check if we actually found a card
  // This is called "defensive programming" - we check for errors before they cause problems
  // The ! operator means "not" - so !card means "if card is falsy (undefined, null, etc.)"
  if (!card) {
    // STEP 3: Handle the error case
    // If no card was found, throw an error with a helpful message
    // This prevents the app from crashing and tells us exactly what went wrong
    // The template literal (backticks) lets us insert the cardIndex into the error message
    throw new Error(`Card not found at index ${cardIndex}`)
  }

  // STEP 4: Return the card (success case)
  // If we found the card, return it to the calling code
